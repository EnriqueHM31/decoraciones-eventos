...
export default function Navegacion() {
    const { isOpenMenuBurguer, handleClickOpenMenuBurguer } = useModalEventos();
        ...
        <div className={`bg-primary w-full flex items-center justify-center backdrop-blur-sm`}>
            <nav className="flex justify-between items-center xl:max-w-11/12 lg:max-w-11/12 max-w-full px-5 lg:px-4 xl:px-0 w-full py-5">
                <div className="text-xl text-white flex items-center gap-2">
                    ...
                    <span className="font-bold xl:text-xl text-sm max-w-1/2 xl:max-w-full">Decoraciones y Entelados</span>
                </div>
                ...
                <ul
                    className={`items-center justify-between 
                                    ...
                                    ${isOpenMenuBurguer ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}
                                    xl:flex xl:w-auto xl:order-1 xl:static xl:translate-x-0 xl:opacity-100 xl:pointer-events-auto`}
                >
                    {LINKS.map((link, index) => (
                        <li key={index} className={`group relative xl:text-xl px-3 xl:py-3 py-2 hover:-translate-y-2.5 duration-500 transition-all  w-full text-center xl:w-fit border-b-4 border-transparent
                            ...
                            >
                                {link.label}
                                <span className={`absolute bottom-0 left-0 h-[2px] w-full bg-white scale-x-0 origin-right transition-transform duration-500 ${location.pathname !== link.to ? "group-hover:scale-x-100 group-hover:origin-left" : ""}`}></span>
                            </a>
                        </li>
                    ))}
                </ul>

                <button className="xl:hidden flex" onClick={handleClickOpenMenuBurguer}
                    title="Abrir menu"
